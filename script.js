...
clipboardEl.addEventListener('click', () => {
    ...
    if (!generatedPassword | generatedPassword == 'check character limits') { return }
...
function generatePassword(length, upper, lower, number, symbol) {
    ...
    const finalPassword = password.split('').sort(function () { return 0.5 - Math.random() }).join('');
    ...
    if (length <= 20 && length > 3) {
        ...
        return resultEl.innerText = 'check character limits'
    }
